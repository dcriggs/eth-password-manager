...
contract PasswordManager {
	...
	modifier onlyAuthenticated(address user, bytes32 hashedPassword) {
		require(
			userRegistration.authenticateUser(user, hashedPassword),
	...
	) external onlyAuthenticated(msg.sender, authenticationHash) {
	...
	function getPasswords(
		...
		external
		view
		onlyAuthenticated(msg.sender, authenticationHash)
